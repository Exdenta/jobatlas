...
description: Search developer jobs and score them for one candidate with the Apify Actor jobatlas/ai-job-fit-scorer. Use to build a bounded search-and-score or score-supplied-jobs input, run it through Apify MCP using latest and validate the resolved immutable build, validate nomad-ai-job-fit-v1 rows and the nomad-ai-job-fit-run-summary-v4 record, and present a shortlist with fit scores, delivery scores, evidence, and gaps.
---

# AI job search and fit scoring

Use this skill for the `jobatlas/ai-job-fit-scorer` contract. This skill
