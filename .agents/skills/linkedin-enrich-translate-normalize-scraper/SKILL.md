...
description: Find and integrate public LinkedIn jobs with the Apify Actor jobatlas/linkedin-enrich-translate-normalize-scraper. Use to build bounded searches, preserve and validate nomad-agent-job-v1 records, connect through Apify MCP, flatten results for tables, or troubleshoot the Actor contract.
---
...
1. Call generic `call-actor` with this outer envelope. Select build `latest` in
   ...
   ```json
   {
     "actor": "jobatlas/linkedin-enrich-translate-normalize-scraper",
